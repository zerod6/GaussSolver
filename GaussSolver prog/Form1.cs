...
namespace GaussSolver
{
    public partial class Form1 : Form
    {
        ...
        // Событие изменения количества переменных, настройка столбцов
        ...
        private void btnSolve_Click(object sender, EventArgs e)
        {
            try
            {
                // Получаем матрицу из DataGridView
                double[,] matrix = GetMatrixFromGrid();
                object solution = null;
                try
                {
                    // Пробуем решить с использованием метода Гаусса
                    solution = Gauss.SolveGaussElimination(matrix);
                }
                catch (InvalidOperationException)
                {
                    // В случае ошибки используем метод наименьших квадратов
                    solution = Gauss.SolveLeastSquares(matrix);
                }

                // Проверка типа решения
                if (solution is string resultMessage)  // Сообщение об ошибке (например, "Бесконечность" или "Нет решений")
                {
                    MessageBox.Show(resultMessage, "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else if (solution is double[] resultArray)  // Верное решение
                {
            ...
            {
                MessageBox.Show("Система несовместна и не имеет решений.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
